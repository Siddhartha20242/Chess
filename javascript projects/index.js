...
function isKingInCheck(playerColor) {
  // Find the position of the king belonging to playerColor
  let kingPosition = [-1, -1];
  for (let i = 0; i < curBoard.length; i++) {
      for (let j = 0; j < curBoard[i].length; j++) {
          if (curBoard[i][j] === (playerColor === 'white' ? 'k' : 'K')) {
              kingPosition = [i, j];
              break;
          }
      }
  ...
  for (let i = 0; i < curBoard.length; i++) {
      for (let j = 0; j < curBoard[i].length; j++) {
          ...
          if ((playerColor === 'white' && piece === piece.toUpperCase()) || (playerColor === 'black' && piece === piece.toLowerCase())) {
              const validMove = validateMovement([i, j], kingPosition);
              if (validMove) {
                  return true;
              }
          }
...
function isCheckmate(playerColor) {
  ...
  // Check if the king can escape or if any other piece can block or capture the attacking piece
  for (let i = 0; i < curBoard.length; i++) {
      for (let j = 0; j < curBoard[i].length; j++) {
          if ((playerColor === 'white' && curBoard[i][j] === 'K') || (playerColor === 'black' && curBoard[i][j] === 'k')) {
              ...
              for (let x = -1; x <= 1; x++) {
                  for (let y = -1; y <= 1; y++) {
                      const newX = i + x;
                      const newY = j + y;
                      if (newX >= 0 && newX < 8 && newY >= 0 && newY < 8 && (x !== 0 || y !== 0) && curBoard[newX][newY] === '.') {
                          const tempBoard = JSON.parse(JSON.stringify(curBoard));
                          tempBoard[newX][newY] = tempBoard[i][j];
                          tempBoard[i][j] = '.';
                          if (!isKingInCheck(playerColor)) {
                              return false;
                          }
              ...
              for (let x = 0; x < 8; x++) {
                  for (let y = 0; y < 8; y++) {
                      if ((playerColor === 'white' && curBoard[x][y] === curBoard[x][y].toUpperCase()) || (playerColor === 'black' && curBoard[x][y] === curBoard[x][y].toLowerCase())) {
                          const validMove = validateMovement([x, y], kingPosition);
                          if (validMove) {
                              const tempBoard = JSON.parse(JSON.stringify(curBoard));
                              tempBoard[kingPosition[0]][kingPosition[1]] = tempBoard[x][y];
                              tempBoard[x][y] = '.';
                              if (!isKingInCheck(playerColor)) {
                                  return false;
                              }
  ...
  if (boardPiece != '.') {
      if ((boardPiece === boardPiece.toUpperCase() && curPlayer == 'black') ||
          (boardPiece === boardPiece.toLowerCase() && curPlayer == 'white')) {
              ...
              if (isKingInCheck(curPlayer === 'white' ? 'black' : 'white')) {
                  if (isCheckmate(curPlayer === 'white' ? 'black' : 'white')) {
                      // End the game, declare winner
                      const winner = curPlayer === 'white' ? 'black' : 'white';
                      alert(`Checkmate! ${winner} wins!`);
                      return;
                  } else {
                      alert(`Check! ${curPlayer === 'white' ? 'black' : 'white'}'s king is in check!`);
                  }
              }
...
function isCheckmate(playerColor) {
  ...
  // Check if the attacking piece can be captured or blocked by other pieces
  for (let i = 0; i < curBoard.length; i++) {
      for (let j = 0; j < curBoard[i].length; j++) {
          if ((playerColor === 'white' && curBoard[i][j] === curBoard[i][j].toUpperCase()) || (playerColor === 'black' && curBoard[i][j] === curBoard[i][j].toLowerCase())) {
              const piecePosition = [i, j];
              const piece = curBoard[i][j];
              if (validateMovement(piecePosition, kingPosition)) {
                  const tempBoard = JSON.parse(JSON.stringify(curBoard));
                  tempBoard[kingPosition[0]][kingPosition[1]] = tempBoard[i][j];
                  tempBoard[i][j] = '.';
                  if (!isKingInCheck(playerColor)) {
                      return false;
                  }
  ...
  // Check if the king can move to escape the check
  for (let x = -1; x <= 1; x++) {
      for (let y = -1; y <= 1; y++) {
          const newX = kingPosition[0] + x;
          const newY = kingPosition[1] + y;
          if (newX >= 0 && newX < 8 && newY >= 0 && newY < 8 && (x !== 0 || y !== 0) && curBoard[newX][newY] === '.') {
              const tempBoard = JSON.parse(JSON.stringify(curBoard));
              tempBoard[newX][newY] = tempBoard[kingPosition[0]][kingPosition[1]];
              tempBoard[kingPosition[0]][kingPosition[1]] = '.';
              if (!isKingInCheck(playerColor)) {
                  return false;
              }
          }
